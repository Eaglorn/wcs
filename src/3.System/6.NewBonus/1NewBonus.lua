...
do
    ...
    function mt:get(unit, type)
        ...
        elseif type == BONUS_MAGIC_RESISTANCE then
            return BlzGetAbilityRealLevelField(BlzGetUnitAbility(unit, MAGIC_RESISTANCE_ABILITY), MAGIC_RESISTANCE_FIELD,
                0)
        elseif type >= BONUS_EVASION_CHANCE and type <= NewBonus.last then
            if NewBonus_EXTENDED and Damage and Evasion and Critical and SpellPower and LifeSteal and SpellVamp and Tenacity then
                ...
                if type == BONUS_CRITICAL_CHANCE then
                    return BlzGetAbilityRealLevelField(BlzGetUnitAbility(unit, CRITICAL_STRIKE_ABILITY),
                        CRITICAL_CHANCE_FIELD, 0)
                elseif type == BONUS_CRITICAL_DAMAGE then
                    return BlzGetAbilityRealLevelField(BlzGetUnitAbility(unit, CRITICAL_STRIKE_ABILITY),
                        CRITICAL_DAMAGE_FIELD, 0)
    ...
    function mt:set(unit, type, value, adding)
        if not adding then
            ...
            if NewBonus.type[NewBonus.key] ~= type then
                return self:set(NewBonus.unit[NewBonus.key], NewBonus.type[NewBonus.key], NewBonus.amount[NewBonus.key],
                    not adding)
            end
        else
            NewBonus.unit[NewBonus.key] = unit
        ...
        elseif type == BONUS_SIGHT_RANGE then
            if value == 0 and not adding then
                BlzSetUnitRealField(unit, UNIT_RF_SIGHT_RADIUS,
                    (BlzGetUnitRealField(unit, UNIT_RF_SIGHT_RADIUS) - self:get(unit, type)))
            else
    ...
    function mt:add(unit, type, value)
        if value ~= 0 then
            ...
            if NewBonus.type[NewBonus.key] <= BONUS_SIGHT_RANGE then
                self:set(NewBonus.unit[NewBonus.key], NewBonus.type[NewBonus.key],
                    self:checkOverflow(self:get(NewBonus.unit[NewBonus.key], NewBonus.type[NewBonus.key]),
                        R2I(NewBonus.amount[NewBonus.key])), true)
            else
                if NewBonus_EXTENDED and Damage and Evasion and Critical and SpellPower and LifeSteal and SpellVamp and Tenacity then
                    if NewBonus.type[NewBonus.key] == BONUS_COOLDOWN_REDUCTION or NewBonus.type[NewBonus.key] == BONUS_TENACITY then
                        self:set(NewBonus.unit[NewBonus.key], NewBonus.type[NewBonus.key], NewBonus.amount[NewBonus.key],
                            true)
                    else
                        self:set(NewBonus.unit[NewBonus.key], NewBonus.type[NewBonus.key],
                            self:get(NewBonus.unit[NewBonus.key], NewBonus.type[NewBonus.key]) +
                            NewBonus.amount[NewBonus.key], true)
                    end
                else
                    self:set(NewBonus.unit[NewBonus.key], NewBonus.type[NewBonus.key],
                        self:get(NewBonus.unit[NewBonus.key], NewBonus.type[NewBonus.key]) +
