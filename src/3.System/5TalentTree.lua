...
do
    ...
    local function ToPascalCase(whichString)
        ...
        return string.upper(whichString:sub(1, 1)) ..
            string.sub(whichString, 2)                                                      --converts first character to upper case
    end
    ...
    local function ShouldTalentBeEnabled(whichTalent, whichPlayer)
        ...
        if REQUIRE_CURRENCY_INSTEAD_OF_POINTS then
            return not playerTalentIsEnabled[whichPlayer][whichTalent]
                ...
                and
                (talentRow[whichTalent] - 1) * POINTS_IN_TREE_REQUIRED_PER_ROW <=
                playerCurrencyInTree[whichPlayer][talentParentTree[whichTalent]]
            ...
            return not playerTalentIsEnabled[whichPlayer][whichTalent]
                ...
                and
                (talentRow[whichTalent] - 1) * POINTS_IN_TREE_REQUIRED_PER_ROW <=
                playerPointsInTree[whichPlayer][talentParentTree[whichTalent]]
            ...
            if talentRequirement[whichTalent] then
                for k = 1, #talentRequirement[whichTalent] do
                    if talentArrowHeadFrame[whichTalent][k] then
                        BlzFrameSetTexture(talentArrowHeadFrame[whichTalent][k],
                            "TalentArrowHead" .. talentRequirementTexture[whichTalent][k] .. "Disabled.blp", 0, true)
                        BlzFrameSetTexture(talentArrowBodyFrame[whichTalent][k],
                            "TalentArrowBody" .. talentRequirementTexture[whichTalent][k] .. "Disabled.blp", 0, true)
                    end
                end
            end
                        ...
                        if talent then
                            if playerTalentIsEnabled[whichPlayer][talent] then
                                ...
                                elseif playerPointsInTalent[whichPlayer][talent] < talentMaxPoints[talent] then
                                    BlzFrameSetEnable(talentFrame[talent], true)
                                    BlzFrameSetTexture(talentIconDisabledFrame[talent],
                                        GetTalentDisabledIconPath(talentIconPath[talent]:gsub(".blp", "")), 0, true)
                                end
                                if talentRequirement[talent] then
                                    for index, requirement in ipairs(talentRequirement[talent]) do
                                        if talentArrowHeadFrame[talent][index] and playerPointsInTalent[whichPlayer][requirement] == talentMaxPoints[requirement] then
                                            BlzFrameSetTexture(talentArrowHeadFrame[talent][index],
                                                "TalentArrowHead" .. talentRequirementTexture[talent][index] .. ".blp", 0,
                                                true)
                                            BlzFrameSetTexture(talentArrowBodyFrame[talent][index],
                                                "TalentArrowBody" .. talentRequirementTexture[talent][index] .. ".blp", 0,
                                                true)
                                        end
                                    end
                                end
    ...
    local function SwitchToTree(whichPlayer, whichTree, whichNavigator)
        if localPlayer == whichPlayer then
            if whichNavigator then
                ...
                if oldTree then
                    BlzFrameSetPoint(treeNavigatorFrame[oldTree], FRAMEPOINT_BOTTOMLEFT, talentBackdrop,
                        FRAMEPOINT_BOTTOMLEFT,
                        TREE_NAVIGATOR_HORIZONTAL_SHIFT +
                        (treeNavigatorPosition[oldTree] - 1) * (TREE_NAVIGATOR_WIDTH - TREE_NAVIGATOR_OVERLAP),
                        TREE_NAVIGATOR_VERTICAL_SHIFT - TREE_NAVIGATOR_HEIGHT)
                    BlzFrameSetPoint(treeNavigatorFrame[oldTree], FRAMEPOINT_TOPRIGHT, talentBackdrop,
                        FRAMEPOINT_BOTTOMLEFT,
                        TREE_NAVIGATOR_HORIZONTAL_SHIFT + treeNavigatorPosition[oldTree] * TREE_NAVIGATOR_WIDTH -
                        (treeNavigatorPosition[oldTree] - 1) * TREE_NAVIGATOR_OVERLAP, TREE_NAVIGATOR_VERTICAL_SHIFT)
                ...
                BlzFrameSetPoint(treeNavigatorFrame[whichTree], FRAMEPOINT_BOTTOMLEFT, talentBackdrop,
                    FRAMEPOINT_BOTTOMLEFT,
                    TREE_NAVIGATOR_HORIZONTAL_SHIFT +
                    (treeNavigatorPosition[whichTree] - 1) * (TREE_NAVIGATOR_WIDTH - TREE_NAVIGATOR_OVERLAP) -
                    TREE_NAVIGATOR_SELECTED_SIZE_INCREASE,
                    TREE_NAVIGATOR_VERTICAL_SHIFT - TREE_NAVIGATOR_HEIGHT - TREE_NAVIGATOR_SELECTED_SIZE_INCREASE)
                BlzFrameSetPoint(treeNavigatorFrame[whichTree], FRAMEPOINT_TOPRIGHT, talentBackdrop,
                    FRAMEPOINT_BOTTOMLEFT,
                    TREE_NAVIGATOR_HORIZONTAL_SHIFT + treeNavigatorPosition[whichTree] * TREE_NAVIGATOR_WIDTH -
                    (treeNavigatorPosition[whichTree] - 1) * TREE_NAVIGATOR_OVERLAP +
                ...
                BlzFrameSetScale(treeNavigatorTitles[whichTree],
                    (TREE_NAVIGATOR_FONT_SIZE / 10) * (TREE_NAVIGATOR_WIDTH + 2 * TREE_NAVIGATOR_SELECTED_SIZE_INCREASE) /
                    TREE_NAVIGATOR_WIDTH)
                if oldTree then
    ...
    local function EnableTalent(whichTalent, whichPlayer)
        ...
        if localPlayer == whichPlayer then
            if playerPointsInTalent[whichPlayer][whichTalent] < talentMaxPoints[whichTalent] then
                ...
                BlzFrameSetText(talentRankFrame[whichTalent],
                    AVAILABLE_COLOR ..
                    playerPointsInTalent[whichPlayer][whichTalent] .. "/" .. talentMaxPoints[whichTalent])
            end

            if talentArrowHeadFrame[whichTalent] then
                for index, __ in ipairs(talentRequirement[whichTalent]) do
                    BlzFrameSetTexture(talentArrowHeadFrame[whichTalent][index],
                        "TalentArrowHead" .. talentRequirementTexture[whichTalent][index] .. ".blp", 0, true)
                    BlzFrameSetTexture(talentArrowBodyFrame[whichTalent][index],
                        "TalentArrowBody" .. talentRequirementTexture[whichTalent][index] .. ".blp", 0, true)
                end
            end
        end
    ...
    local function GetRequirementsText(whichPlayer, whichTalent)
        ...
        if talentRequirement[whichTalent] then
            for __, requirement in ipairs(talentRequirement[whichTalent]) do
                if requirement and playerPointsInTalent[whichPlayer][requirement] < talentMaxPoints[requirement] then
                    text = text ..
                        REQUIREMENT_NOT_MET_TEXT:gsub("!REQUIREMENT!", requirement):gsub("!MAXPOINTS!",
                            talentMaxPoints[requirement]):gsub("\x25(s\x25)",
                            talentMaxPoints[requirement] > 1 and "s" or "") ..
                        "|n"
                end
            end
        end
        ...
        if (REQUIRE_CURRENCY_INSTEAD_OF_POINTS and pointsRequired > playerCurrencyInTree[whichPlayer][whichTree]) or (not REQUIRE_CURRENCY_INSTEAD_OF_POINTS and pointsRequired > playerPointsInTree[whichPlayer][whichTree]) then
            text = text ..
                NOT_ENOUGH_POINTS_IN_TREE_TEXT:gsub("!TREEPOINTS!", pointsRequired):gsub("!TREE!", whichTree):gsub(
                    "\x25(s\x25)", pointsRequired > 1 and "s" or "") .. "|n"
        end
        ...
        if (REQUIRE_CURRENCY_INSTEAD_OF_POINTS and pointsRequired > playerCurrencyTotal[whichPlayer]) or (not REQUIRE_CURRENCY_INSTEAD_OF_POINTS and pointsRequired > playerPointsTotal[whichPlayer]) then
            text = text ..
                NOT_ENOUGH_POINTS_TOTAL_TEXT:gsub("!TOTALPOINTS!", pointsRequired):gsub("\x25(s\x25)",
                    pointsRequired > 1 and "s" or "") .. "|n"
        end
        return text
    end
    ...
    local function GetTalentTooltip(whichPlayer, whichTalent)
        ...
        if talentTooltip[whichTalent][0] and currentLevel == 0 then
            return ((requirementsText and requirementsText .. "|n" .. (overwrite and "" or GetRequirementsText(whichPlayer, whichTalent))) or GetRequirementsText(whichPlayer, whichTalent)) ..
                TALENT_TITLE:gsub("!TREE!", talentParentTree[whichTalent]) ..
                "|n" ..
                NEXT_RANK_TEXT:gsub("!POINTS!", 1):gsub("!MAXPOINTS!", maxPoints) ..
                "|n" .. talentTooltip[whichTalent][0] .. GetAffectsText(whichTalent)
        elseif maxPoints == 1 then
            return ((requirementsText and requirementsText .. "|n" .. (overwrite and "" or GetRequirementsText(whichPlayer, whichTalent))) or GetRequirementsText(whichPlayer, whichTalent)) ..
                TALENT_TITLE:gsub("!TREE!", talentParentTree[whichTalent]) ..
                "|n" .. talentTooltip[whichTalent][1] .. GetAffectsText(whichTalent)
        elseif currentLevel == maxPoints then
            return ((requirementsText and requirementsText .. "|n" .. (overwrite and "" or GetRequirementsText(whichPlayer, whichTalent))) or GetRequirementsText(whichPlayer, whichTalent)) ..
                TALENT_TITLE:gsub("!TREE!", talentParentTree[whichTalent]) ..
                "|n" ..
                CURRENT_RANK_TEXT:gsub("!POINTS!", currentLevel):gsub("!MAXPOINTS!", maxPoints) ..
                "|n" .. talentTooltip[whichTalent][maxPoints] .. GetAffectsText(whichTalent)
        elseif currentLevel == 0 then
            ...
            return ((requirementsText and requirementsText .. "|n" .. (overwrite and "" or GetRequirementsText(whichPlayer, whichTalent))) or GetRequirementsText(whichPlayer, whichTalent)) ..
                ...
                CURRENT_RANK_TEXT:gsub("!POINTS!", currentLevel):gsub("!MAXPOINTS!", maxPoints) ..
                "|n" ..
                talentTooltip[whichTalent][currentLevel] ..
                "|n|n" ..
                NEXT_RANK_TEXT:gsub("!POINTS!", currentLevel + 1):gsub("!MAXPOINTS!", maxPoints) ..
                "|n" .. talentTooltip[whichTalent][currentLevel + 1] .. GetAffectsText(whichTalent)
        end
    end
    ...
    local function SetTalentTooltip(whichPlayer, whichTalent)
        if localPlayer == whichPlayer then
            ...
            BlzFrameSetSize(talentTooltipFrame[whichTalent], 0.29,
                BlzFrameGetHeight(talentTooltipTextFrame[whichTalent]) + TALENT_TOOLTIP_HEIGHT_BUFFER)
        end
    end
    ...
    local function ResetTalent(whichPlayer, whichTalent)
        if localPlayer == whichPlayer then
            BlzFrameSetEnable(talentFrame[whichTalent], false)
            BlzFrameSetTexture(talentIconDisabledFrame[whichTalent],
                GetTalentDisabledIconPath(talentIconPath[whichTalent]:gsub(".blp", "")), 0, true)
            BlzFrameSetVisible(talentRankBoxFrame[whichTalent], true)
            BlzFrameSetVisible(talentRankFrame[whichTalent], true)
            BlzFrameSetText(talentRankFrame[whichTalent],
                UNAVAILABLE_COLOR .. "0/" .. talentMaxPoints[whichTalent] .. "|r")
            ...
            if talentArrowHeadFrame[whichTalent] then
                for i = 1, #talentRequirement[whichTalent] do
                    if talentArrowHeadFrame[whichTalent][i] then
                        BlzFrameSetTexture(talentArrowHeadFrame[whichTalent][i],
                            "TalentArrowHead" .. talentRequirementTexture[whichTalent][i] .. "Disabled.blp", 0, true)
                        BlzFrameSetTexture(talentArrowBodyFrame[whichTalent][i],
                            "TalentArrowBody" .. talentRequirementTexture[whichTalent][i] .. "Disabled.blp", 0, true)
                    end
                end
            end
        end
    ...
    local function SelectTalent(whichPlayer, whichTalent)
        ...
        if REQUIRE_CURRENCY_INSTEAD_OF_POINTS then
            local cost = GetTalentCost(whichTalent, talentMaxPoints[whichTalent],
                playerPointsInTalent[whichPlayer][whichTalent])
        ...
        if talentOnLearn[whichTalent] then
            talentOnLearn[whichTalent](whichPlayer, whichTalent, whichTree,
                playerPointsInTalent[whichPlayer][whichTalent] - 1, playerPointsInTalent[whichPlayer][whichTalent])
        end

        if GLOBAL_CALLBACK then
            GLOBAL_CALLBACK(whichPlayer, whichTalent, whichTree, playerPointsInTalent[whichPlayer][whichTalent] - 1,
                playerPointsInTalent[whichPlayer][whichTalent])
        end
        ...
        if playerPointsInTalent[whichPlayer][whichTalent] == talentMaxPoints[whichTalent] then
            if localPlayer == whichPlayer then
                BlzFrameSetEnable(talentFrame[whichTalent], false)
                BlzFrameSetTexture(talentIconDisabledFrame[whichTalent],
                    GetTalentMaxedIconPath(talentIconPath[whichTalent]:gsub(".blp", "")), 0, true)
                BlzFrameSetText(talentRankFrame[whichTalent],
                    MAXED_COLOR .. talentMaxPoints[whichTalent] .. "/" .. talentMaxPoints[whichTalent] .. "|r")
            ...
            if localPlayer == whichPlayer then
                BlzFrameSetText(talentRankFrame[whichTalent],
                    AVAILABLE_COLOR ..
                    playerPointsInTalent[whichPlayer][whichTalent] .. "/" .. talentMaxPoints[whichTalent] .. "|r")
                if VARIABLE_TALENT_COST then
                    BlzFrameSetText(talentTooltipCostFrame[whichTalent],
                        tostring(GetTalentCost(whichTalent, talentMaxPoints[whichTalent],
                            playerPointsInTalent[whichPlayer][whichTalent])))
                end
            end
        end
    ...
    local function TalentOnClick()
        ...
        local cost = VARIABLE_TALENT_COST and
            GetTalentCost(whichTalent, talentMaxPoints[whichTalent], playerPointsInTalent[whichPlayer][whichTalent]) or 1
        ...
        if talentRow[whichTalent] == talentRow[requirement] then
            if talentColumn[whichTalent] == talentColumn[requirement] - 1 then
                return "Left", FRAMEPOINT_BOTTOMRIGHT,
                    ...
                    -REQUIREMENT_ARROW_BUTTON_OVERLAP + REQUIREMENT_ARROW_WIDTH,
                    TALENT_BUTTON_SIZE / 2 + REQUIREMENT_ARROW_WIDTH / 2,
                    -REQUIREMENT_ARROW_BUTTON_OVERLAP + REQUIREMENT_ARROW_WIDTH,
                    TALENT_BUTTON_SIZE / 2 - REQUIREMENT_ARROW_WIDTH / 2,
            ...
            if TALENT_TREE_BOTTOM_TO_TOP then
                if talentRow[whichTalent] == talentRow[requirement] + 1 then
                    return "Up", FRAMEPOINT_BOTTOMLEFT,
                        ...
                        TALENT_BUTTON_SIZE / 2 - REQUIREMENT_ARROW_WIDTH / 2,
                        -TALENT_VERTICAL_SPACING + TALENT_BUTTON_SIZE,
                        TALENT_BUTTON_SIZE / 2 + REQUIREMENT_ARROW_WIDTH / 2,
                        REQUIREMENT_ARROW_BUTTON_OVERLAP - REQUIREMENT_ARROW_WIDTH
                elseif talentRow[whichTalent] == talentRow[requirement] + 2 then
                    return "Up", FRAMEPOINT_BOTTOMLEFT,
                        ...
                        TALENT_BUTTON_SIZE / 2 + REQUIREMENT_ARROW_WIDTH / 2, REQUIREMENT_ARROW_BUTTON_OVERLAP,
                        TALENT_BUTTON_SIZE / 2 - REQUIREMENT_ARROW_WIDTH / 2,
                        -2 * TALENT_VERTICAL_SPACING + TALENT_BUTTON_SIZE - REQUIREMENT_ARROW_WIDTH,
                        TALENT_BUTTON_SIZE / 2 + REQUIREMENT_ARROW_WIDTH / 2, REQUIREMENT_ARROW_BUTTON_OVERLAP
                end
            else
                if talentRow[whichTalent] == talentRow[requirement] + 1 then
                ...
                elseif talentRow[whichTalent] == talentRow[requirement] + 2 then
                    return "Down", FRAMEPOINT_TOPLEFT,
                        ...
                        -REQUIREMENT_ARROW_BUTTON_OVERLAP + REQUIREMENT_ARROW_WIDTH,
                        TALENT_BUTTON_SIZE / 2 + REQUIREMENT_ARROW_WIDTH / 2,
                        2 * TALENT_VERTICAL_SPACING - TALENT_BUTTON_SIZE
                end
            end
        end
    end
        ...
        for i = 1, NUMBER_OF_TALENT_COLUMNS do
            for j = 1, NUMBER_OF_TALENT_ROWS do
                ...
                if whichTalent ~= nil then
                    ...
                    BlzFrameSetPoint(talentFrame[whichTalent], FRAMEPOINT_TOPRIGHT, parent, FRAMEPOINT_BOTTOMLEFT,
                        x + TALENT_BUTTON_SIZE, y + TALENT_BUTTON_SIZE)
                    ...
                    BlzFrameSetPoint(talentIconClickedFrame[whichTalent], FRAMEPOINT_BOTTOMLEFT, talentFrame
                        [whichTalent], FRAMEPOINT_BOTTOMLEFT, 0.001, 0.001)
                    BlzFrameSetPoint(talentIconClickedFrame[whichTalent], FRAMEPOINT_TOPRIGHT, talentFrame[whichTalent],
                        FRAMEPOINT_TOPRIGHT, -0.001, -0.001)
                    ...
                    BlzFrameSetAbsPoint(talentTooltipFrame[whichTalent], FRAMEPOINT_BOTTOMLEFT,
                        math.min(TREE_MENU_X - TREE_MENU_WIDTH / 2 + x + TALENT_BUTTON_SIZE, 0.51),
                        TREE_MENU_Y - TREE_MENU_HEIGHT / 2 + y + TALENT_BUTTON_SIZE)
                    ...
                    BlzFrameSetPoint(talentRankBoxFrame[whichTalent], FRAMEPOINT_BOTTOMLEFT, talentFrame[whichTalent],
                        FRAMEPOINT_BOTTOMRIGHT, -RANK_INDICATOR_HORIZONTAL_OVERLAP,
                        RANK_INDICATOR_VERTICAL_OVERLAP - RANK_INDICATOR_HEIGHT)
                    BlzFrameSetPoint(talentRankBoxFrame[whichTalent], FRAMEPOINT_TOPRIGHT, talentFrame[whichTalent],
                        FRAMEPOINT_BOTTOMRIGHT, -RANK_INDICATOR_HORIZONTAL_OVERLAP + RANK_INDICATOR_WIDTH,
                        RANK_INDICATOR_VERTICAL_OVERLAP)
                    ...
                    for p = 0, 23 do
                        ...
                        if ShouldTalentBeEnabled(whichTalent, whichPlayer) then
                            BlzFrameSetText(talentRankFrame[whichTalent],
                                AVAILABLE_COLOR .. "0/" .. talentMaxPoints[whichTalent] .. "|r")
                            playerTalentIsEnabled[whichPlayer][whichTalent] = true
                        else
                            BlzFrameSetText(talentRankFrame[whichTalent],
                                UNAVAILABLE_COLOR .. "0/" .. talentMaxPoints[whichTalent] .. "|r")
                    ...
                    if VARIABLE_TALENT_COST then
                        BlzFrameSetTextAlignment(talentTooltipCostFrame[whichTalent], TEXT_JUSTIFY_MIDDLE,
                            TEXT_JUSTIFY_RIGHT)
                        BlzFrameSetText(talentTooltipCostFrame[whichTalent],
                            tostring(GetTalentCost(whichTalent, talentMaxPoints[whichTalent], 0)))
                    else
                        BlzFrameSetVisible(talentTooltipCostFrame[whichTalent], false)
                        BlzFrameSetVisible(talentTooltipIconFrame[whichTalent], false)
                    end
                    ...
                    if talentRequirement[whichTalent] then
                        ...
                        for index, requirement in ipairs(talentRequirement[whichTalent]) do
                            local texture, framepoint, x1h, y1h, x2h, y2h, x1b, y1b, x2b, y2b = GetArrowFrameParameters(
                                whichTalent, requirement)
                            if texture then
                                talentRequirementTexture[whichTalent][index] = texture

                                talentArrowHeadFrame[whichTalent][index] = BlzCreateFrameByType("BACKDROP",
                                    "talentArrowHeadFrame", talentFrame[whichTalent], "", 0)
                                BlzFrameSetTexture(talentArrowHeadFrame[whichTalent][index],
                                    "TalentArrowHead" .. texture .. "Disabled.blp", 0, true)
                                BlzFrameSetPoint(talentArrowHeadFrame[whichTalent][index], FRAMEPOINT_BOTTOMLEFT,
                                    talentFrame[whichTalent], framepoint, x1h, y1h)
                                BlzFrameSetPoint(talentArrowHeadFrame[whichTalent][index], FRAMEPOINT_TOPRIGHT,
                                    talentFrame[whichTalent], framepoint, x2h, y2h)
                                BlzFrameSetEnable(talentArrowHeadFrame[whichTalent][index], false)

                                talentArrowBodyFrame[whichTalent][index] = BlzCreateFrameByType("BACKDROP",
                                    "talentArrowBodyFrame", talentFrame[whichTalent], "", 0)
                                BlzFrameSetTexture(talentArrowBodyFrame[whichTalent][index],
                                    "TalentArrowBody" .. texture .. "Disabled.blp", 0, true)
                                BlzFrameSetPoint(talentArrowBodyFrame[whichTalent][index], FRAMEPOINT_BOTTOMLEFT,
                                    talentFrame[whichTalent], framepoint, x1b, y1b)
                                BlzFrameSetPoint(talentArrowBodyFrame[whichTalent][index], FRAMEPOINT_TOPRIGHT,
                                    talentFrame[whichTalent], framepoint, x2b, y2b)
    ...
    local function CreateTalent(data)
        ...
        if talentFromPosition[tree][column][row] then
            print("|cffff0000Warning:|r Multiple talents declared for column " ..
                column .. ", row " .. row .. " in talent tree " .. tree .. ".")
            return
        end
        ...
        if requirement then
            ...
            for __, req in ipairs(talentRequirement[name]) do
                if talentTooltip[req] == nil then
                    print("|cffff0000Warning:|r Requirement talent " ..
                        req .. "  not recognized. Defined before requirement?")
                    return
                end
            end
        end
                ...
                for i = 0, maxPoints do
                    if talentTooltip[name][i] then
                        ...
                        if type(value) == "table" then
                            if isPercent then
                                talentTooltip[name][i] = talentTooltip[name][i]:gsub("!" .. key .. ",\x25\x25!",
                                    TOOLTIP_VALUE_COLOR .. math.floor(100 * value[v] + 0.5) .. "\x25\x25|r")
                            else
                                talentTooltip[name][i] = talentTooltip[name][i]:gsub("!" .. key .. "!",
                                    TOOLTIP_VALUE_COLOR .. (math.tointeger(value[v]) or value[v]) .. "|r")
                            end
                        ...
                        elseif isFromField then
                            ...
                            if isPercent then
                                talentTooltip[name][i] = talentTooltip[name][i]:gsub("!" .. key .. ",\x25\x25!",
                                    TOOLTIP_VALUE_COLOR .. math.floor(100 * fieldValue + 0.5) .. "\x25\x25|r")
                            else
                                talentTooltip[name][i] = talentTooltip[name][i]:gsub("!" .. key .. "!",
                                    TOOLTIP_VALUE_COLOR .. (math.tointeger(fieldValue) or fieldValue) .. "|r")
                            end
                            talentValues[name][key][i] = fieldValue
                        elseif isPercent then
                            if isStatic then
                                talentTooltip[name][i] = talentTooltip[name][i]:gsub("!" .. key .. ",\x25\x25\x25$",
                                    TOOLTIP_VALUE_COLOR .. math.floor(100 * value + 0.5) .. "\x25\x25|r")
                                talentTooltip[name][i] = talentTooltip[name][i]:gsub("!" .. key .. ",\x25$\x25\x25",
                                    TOOLTIP_VALUE_COLOR .. math.floor(100 * value + 0.5) .. "\x25\x25|r")
                                talentValues[name][key][i] = value
                            else
                                talentTooltip[name][i] = talentTooltip[name][i]:gsub("!" .. key .. ",\x25\x25!",
                                    TOOLTIP_VALUE_COLOR .. math.floor(100 * v * value + 0.5) .. "\x25\x25|r")
                                talentValues[name][key][i] = v * value
                            end
                        else
                            if isStatic then
                                talentTooltip[name][i] = talentTooltip[name][i]:gsub("!" .. key .. ",\x25$!",
                                    TOOLTIP_VALUE_COLOR .. (math.tointeger(value) or value) .. "|r")
                                talentValues[name][key][i] = value
                            else
                                talentTooltip[name][i] = talentTooltip[name][i]:gsub("!" .. key .. "!",
                                    TOOLTIP_VALUE_COLOR .. (math.tointeger(v * value) or v * value) .. "|r")
                                talentValues[name][key][i] = v * value
                            end
                        end
                    end
    ...
    local function CreateTalentTree(treeName)
        talentTreeParentFrame[treeName] = BlzCreateFrame("TalentTreeParentFrame",
            BlzGetFrameByName("ConsoleUIBackdrop", 0), 0, 0)
        BlzFrameSetAbsPoint(talentTreeParentFrame[treeName], FRAMEPOINT_TOPLEFT, TREE_MENU_X - TREE_MENU_WIDTH / 2,
            TREE_MENU_Y + TREE_MENU_HEIGHT / 2)
        BlzFrameSetAbsPoint(talentTreeParentFrame[treeName], FRAMEPOINT_BOTTOMRIGHT, TREE_MENU_X + TREE_MENU_WIDTH / 2,
            TREE_MENU_Y - TREE_MENU_HEIGHT / 2)
        ...
        BlzFrameSetPoint(backdrop, FRAMEPOINT_BOTTOMLEFT, talentTreeParentFrame[treeName], FRAMEPOINT_BOTTOMLEFT,
            BACKGROUND_HORIZONTAL_INSET, BACKGROUND_INSET_BOTTOM)
        BlzFrameSetPoint(backdrop, FRAMEPOINT_TOPRIGHT, talentTreeParentFrame[treeName], FRAMEPOINT_TOPRIGHT,
            -BACKGROUND_HORIZONTAL_INSET, -BACKGROUND_INSET_TOP)
        BlzFrameSetEnable(backdrop, false)

        local bottomBar = BlzFrameGetChild(talentTreeParentFrame[treeName], 1)
        BlzFrameSetPoint(bottomBar, FRAMEPOINT_BOTTOMLEFT, talentTreeParentFrame[treeName], FRAMEPOINT_BOTTOMLEFT,
            BOTTOM_BAR_HORIZONTAL_INSET, BOTTOM_BAR_VERTICAL_SHIFT)
        BlzFrameSetPoint(bottomBar, FRAMEPOINT_TOPRIGHT, talentTreeParentFrame[treeName], FRAMEPOINT_BOTTOMRIGHT,
            -BOTTOM_BAR_HORIZONTAL_INSET, BOTTOM_BAR_HEIGHT + BOTTOM_BAR_VERTICAL_SHIFT)
        BlzFrameSetEnable(bottomBar, false)

        local topBar = BlzFrameGetChild(talentTreeParentFrame[treeName], 2)
        BlzFrameSetPoint(topBar, FRAMEPOINT_BOTTOMLEFT, talentTreeParentFrame[treeName], FRAMEPOINT_TOPLEFT,
            TOP_BAR_HORIZONTAL_INSET, TOP_BAR_VERTICAL_SHIFT - TOP_BAR_HEIGHT)
        BlzFrameSetPoint(topBar, FRAMEPOINT_TOPRIGHT, talentTreeParentFrame[treeName], FRAMEPOINT_TOPRIGHT,
            -TOP_BAR_HORIZONTAL_INSET, TOP_BAR_VERTICAL_SHIFT)
        ...
        BlzFrameSetPoint(closeButton, FRAMEPOINT_BOTTOMLEFT, header, FRAMEPOINT_BOTTOMRIGHT,
            CLOSE_BUTTON_HORIZONTAL_SHIFT - CLOSE_BUTTON_SIZE, CLOSE_BUTTON_VERTICAL_SHIFT)
        BlzFrameSetPoint(closeButton, FRAMEPOINT_TOPRIGHT, header, FRAMEPOINT_BOTTOMRIGHT, CLOSE_BUTTON_HORIZONTAL_SHIFT,
            CLOSE_BUTTON_VERTICAL_SHIFT + CLOSE_BUTTON_SIZE)
        ...
        BlzFrameSetAbsPoint(border, FRAMEPOINT_TOPLEFT, TREE_MENU_X - TREE_MENU_WIDTH / 2,
            TREE_MENU_Y + TREE_MENU_HEIGHT / 2)
        BlzFrameSetAbsPoint(border, FRAMEPOINT_BOTTOMRIGHT, TREE_MENU_X + TREE_MENU_WIDTH / 2,
            TREE_MENU_Y - TREE_MENU_HEIGHT / 2)
        BlzFrameSetEnable(border, false)

        talentTreePointsSpentText[treeName] = BlzCreateFrameByType("TEXT", "talentTreePointsSpent" .. treeName, border,
            "", 0)
        BlzFrameSetPoint(talentTreePointsSpentText[treeName], FRAMEPOINT_BOTTOMLEFT, border, FRAMEPOINT_BOTTOMLEFT,
            BOTTOM_LEFT_TEXT_HORIZONTAL_INSET, BOTTOM_BAR_VERTICAL_SHIFT + BOTTOM_BAR_TEXT_VERTICAL_SHIFT)
        BlzFrameSetPoint(talentTreePointsSpentText[treeName], FRAMEPOINT_TOPRIGHT, border, FRAMEPOINT_BOTTOMRIGHT,
            -BOTTOM_LEFT_TEXT_HORIZONTAL_INSET,
            BOTTOM_BAR_HEIGHT + BOTTOM_BAR_VERTICAL_SHIFT + BOTTOM_BAR_TEXT_VERTICAL_SHIFT)
        ...
        talentTreeUnspentPointsText[treeName] = BlzCreateFrameByType("TEXT", "talentTreePointsSpent" .. treeName, border,
            "", 0)
        BlzFrameSetPoint(talentTreeUnspentPointsText[treeName], FRAMEPOINT_BOTTOMLEFT, border, FRAMEPOINT_BOTTOMRIGHT,
            -BOTTOM_RIGHT_TEXT_HORIZONTAL_INSET, BOTTOM_BAR_VERTICAL_SHIFT + BOTTOM_BAR_TEXT_VERTICAL_SHIFT)
        BlzFrameSetPoint(talentTreeUnspentPointsText[treeName], FRAMEPOINT_TOPRIGHT, border, FRAMEPOINT_BOTTOMRIGHT, 0,
            BOTTOM_BAR_HEIGHT + BOTTOM_BAR_VERTICAL_SHIFT + BOTTOM_BAR_TEXT_VERTICAL_SHIFT)
        ...
        if BLACK_FADE_CORNER_SIZE > 0 then
            ...
            for i = 1, 4 do
                blackFadeFrames[i] = BlzCreateFrameByType("BACKDROP", "blackFadeFrame" .. i,
                    talentTreeParentFrame[treeName], "", 0)
                BlzFrameSetTexture(blackFadeFrames[i], "TalentTreeFade" .. i .. ".blp", 0, true)
                BlzFrameSetEnable(blackFadeFrames[i], false)
            end
            BlzFrameSetSize(blackFadeFrames[1], BLACK_FADE_CORNER_SIZE,
                TREE_MENU_HEIGHT - BACKGROUND_INSET_BOTTOM - BACKGROUND_INSET_TOP)
            BlzFrameSetPoint(blackFadeFrames[1], FRAMEPOINT_BOTTOMRIGHT, talentTreeParentFrame[treeName],
                FRAMEPOINT_BOTTOMRIGHT, -BACKGROUND_HORIZONTAL_INSET, BACKGROUND_INSET_BOTTOM)

            BlzFrameSetSize(blackFadeFrames[2],
                TREE_MENU_WIDTH - BACKGROUND_HORIZONTAL_INSET - BACKGROUND_HORIZONTAL_INSET, BLACK_FADE_CORNER_SIZE)
            BlzFrameSetPoint(blackFadeFrames[2], FRAMEPOINT_TOPLEFT, talentTreeParentFrame[treeName], FRAMEPOINT_TOPLEFT,
                BACKGROUND_HORIZONTAL_INSET, -BACKGROUND_INSET_TOP)

            BlzFrameSetSize(blackFadeFrames[3], BLACK_FADE_CORNER_SIZE,
                TREE_MENU_HEIGHT - BACKGROUND_INSET_BOTTOM - BACKGROUND_INSET_TOP)
            BlzFrameSetPoint(blackFadeFrames[3], FRAMEPOINT_BOTTOMLEFT, talentTreeParentFrame[treeName],
                FRAMEPOINT_BOTTOMLEFT, BACKGROUND_HORIZONTAL_INSET, BACKGROUND_INSET_BOTTOM)

            BlzFrameSetSize(blackFadeFrames[4],
                TREE_MENU_WIDTH - BACKGROUND_HORIZONTAL_INSET - BACKGROUND_HORIZONTAL_INSET, BLACK_FADE_CORNER_SIZE)
            BlzFrameSetPoint(blackFadeFrames[4], FRAMEPOINT_BOTTOMLEFT, talentTreeParentFrame[treeName],
                FRAMEPOINT_BOTTOMLEFT, BACKGROUND_HORIZONTAL_INSET, BACKGROUND_INSET_BOTTOM)
        end
    ...
    OnInit.final("TalentTree", function()
        ...
        BlzFrameSetAbsPoint(talentBackdrop, FRAMEPOINT_BOTTOMLEFT, TREE_MENU_X - TREE_MENU_WIDTH / 2,
            TREE_MENU_Y - TREE_MENU_HEIGHT / 2)
        BlzFrameSetAbsPoint(talentBackdrop, FRAMEPOINT_TOPRIGHT, TREE_MENU_X + TREE_MENU_WIDTH / 2,
            TREE_MENU_Y + TREE_MENU_HEIGHT / 2)
    ...
    CTT.RegisterTalent = function(data)
        ...
        for key, __ in pairs(data) do
            if not RECOGNIZED_FLAGS[key] then
                print("|cffff0000Warning:|r Unrecognized flag " ..
                    key .. " in data table passed into RegisterTalent function.")
            end
        end
        for key, __ in ipairs(REQUIRED_FLAGS) do
            if not data[key] then
                print("|cffff000Warning:|r Required flag " ..
                    key .. " missing in data table passed into RegisterTalent function.")
                return
            end
        end
        ...
        if not data.fourCC and not (data.name and data.icon and data.tooltip) then
            print(
                "|cffff000Warning:|r Data table for RegisterTalent function requires either fourCC or name, tooltip, and icon.")
            return
        end
    ...
    CTT.SetTrees = function(who, ...)
        ...
        for i = 1, select("#", ...) do
            ...
            BlzFrameSetPoint(treeNavigatorFrame[tree], FRAMEPOINT_BOTTOMLEFT, talentBackdrop, FRAMEPOINT_BOTTOMLEFT,
                TREE_NAVIGATOR_HORIZONTAL_SHIFT + (i - 1) * (TREE_NAVIGATOR_WIDTH - TREE_NAVIGATOR_OVERLAP),
                TREE_NAVIGATOR_VERTICAL_SHIFT - TREE_NAVIGATOR_HEIGHT)
            BlzFrameSetPoint(treeNavigatorFrame[tree], FRAMEPOINT_TOPRIGHT, talentBackdrop, FRAMEPOINT_BOTTOMLEFT,
                TREE_NAVIGATOR_HORIZONTAL_SHIFT + i * TREE_NAVIGATOR_WIDTH - (i - 1) * TREE_NAVIGATOR_OVERLAP,
                TREE_NAVIGATOR_VERTICAL_SHIFT)
    ...
    CTT.ForceSelectTalent = function(who, whichTalent, subtractCost)
        ...
        if subtractCost then
            local cost = VARIABLE_TALENT_COST and
                GetTalentCost(whichTalent, talentMaxPoints[whichTalent], playerPointsInTalent[who][whichTalent]) or 1
    ...
    CTT.DisableTalent = function(who, whichTalent, requirementsText, overwriteRequirements)
        ...
        if not playerTalentIsHardDisabled[who][whichTalent] then
            ...
            BlzFrameSetText(talentRankFrame[whichTalent],
                UNAVAILABLE_COLOR .. playerPointsInTalent[who][whichTalent] .. "/" .. talentMaxPoints[whichTalent])
    ...
    CTT.GetImageSizeRatio = function(width, height)
        local ratio = (TREE_MENU_HEIGHT - BOTTOM_BAR_HEIGHT - TOP_BAR_HEIGHT - BOTTOM_BAR_VERTICAL_SHIFT + TOP_BAR_VERTICAL_SHIFT - 2 * BACKGROUND_VERTICAL_INSET) /
            (TREE_MENU_WIDTH - 2 * BACKGROUND_HORIZONTAL_INSET)
        local text = "The optimal image size ratio is: |cffffcc00" .. string.format("\x25.3f", ratio) .. "|r."
        if width and not height then
            text = text ..
                " At a fixed width of " ..
                width .. ", this requires a height of |cffffcc00" .. math.floor(width * ratio + 0.5) .. "|r."
        elseif height and not width then
            text = text ..
                " At a fixed height of " ..
                height .. ", this requires a width of |cffffcc00" .. math.floor(height / ratio + 0.5) .. "|r."
