...
do
    ...
    local toCamelCase = setmetatable({}, {
        __index = function(self, whichString)
            ...
            self[whichString] = string.lower(whichString:sub(1, 1)) ..
            string.sub(whichString, 2)                                                          --converts first character to lower case
    ...
    local function Object2String(object)
        if IsHandle[object] then
            if IsWidget[object] then
                ...
                return HandleType[object] ..
                " " .. (address:sub(1, 1) == "0" and address:sub(address:len() - 3, address:len()))
            end
    ...
    local coord = {
        ...
        classZ        = setmetatable({},
            { __index = function(self, key)
                self[key] = key.anchor.z + key.zOffset
                return self[key]
            end }),
        unitX         = setmetatable({},
            { __index = function(self, key)
                self[key] = GetUnitX(key)
                return self[key]
            end }),
        unitY         = setmetatable({},
            { __index = function(self, key)
                self[key] = GetUnitY(key)
                return self[key]
            end }),
        unitZ         = setmetatable({},
            { __index = function(self, key)
                self[key] = GetTerrainZ(key.x[key.anchor], key.y[key.anchor]) + GetUnitFlyHeight(key.anchor) +
                key.zOffset
                return self[key]
            end }),
        destructableX = setmetatable({},
            { __index = function(self, key)
                self[key] = GetDestructableX(key)
                return self[key]
            end }),
        destructableY = setmetatable({},
            { __index = function(self, key)
                self[key] = GetDestructableY(key)
                return self[key]
            end }),
        destructableZ = setmetatable({},
            { __index = function(self, key)
                self[key] = GetTerrainZ(key.x[key.anchor], key.y[key.anchor]) + key.zOffset
                return self[key]
            end }),
        itemX         = setmetatable({},
            { __index = function(self, key)
                self[key] = GetItemX(key)
                return self[key]
            end }),
        itemY         = setmetatable({},
            { __index = function(self, key)
                self[key] = GetItemY(key)
                return self[key]
            end }),
        itemZ         = setmetatable({},
            { __index = function(self, key)
        ...
        terrainZ      = setmetatable({},
            { __index = function(self, key)
                self[key] = GetTerrainZ(key.x[key.anchor], key.y[key.anchor]) + key.zOffset
                return self[key]
            end }),
        globalXYZ     = setmetatable({},
            { __index = function(self, key)
                self[key] = 0
                return 0
            end })
    ...
    local function GetInteractionFunc(male, female)
        ...
        if conflict then
            error("InteractionFunc ambiguous for actors with identifiers " ..
            Identifier2String(male.identifier) .. " and " .. Identifier2String(female.identifier) .. ".")
        end
        if func then
            ...
            if functionDelay[interactionFunc] then
                if functionDelayIsDistributed[interactionFunc] then
                    ...
                    if functionDelayCurrent[interactionFunc] > functionDelay[interactionFunc] then
                        functionDelayCurrent[interactionFunc] = functionDelayCurrent[interactionFunc] -
                        functionDelay[interactionFunc]
                    end
    ...
    Create = function(host, identifier, interactions, flags)
        ...
        if flags.isAnonymous then
            if interactions then
                error(
                "An anonymous actor cannot carry an interactions table. To add self-interactions, use the selfInteractions flag.")
            end
        ...
        if not self.isGlobal then
            ...
            if self.isStationary then
                ...
                local interval = additionalFlags.cellCheckInterval or flags.cellCheckInterval or
                config.DEFAULT_CELL_CHECK_INTERVAL
                self.cellCheckInterval = min(MAX_STEPS, max(1, (interval * INV_MIN_INTERVAL) // 1 + 1))
            else
                InitCellChecks(self,
                    additionalFlags.cellCheckInterval or flags.cellCheckInterval or config.DEFAULT_CELL_CHECK_INTERVAL)
            end
        end
    ...
    CreateBinds = function(self, bindToBuff, bindToOrder)
        ...
        elseif HandleType[self.anchor] == "unit" then
            ...
            Warning("|cffff0000Warning:|r Attempted to bind actor with identifier " ..
            Identifier2String(self.identifier) ..
            " to a buff or order, but that actor doesn't have a unit host or anchor.")
        end
    end
    ...
    GetDescription = function(self)
        local description = setmetatable({}, { __add = function(old, new)
            old[#old + 1] = new
            return old
        end })
        ...
        if self.cellCheckInterval and math.abs(self.cellCheckInterval * config.MIN_INTERVAL - config.DEFAULT_CELL_CHECK_INTERVAL) > 0.001 then
            description = description + "\n|cffffcc00Cell Check Interval:|r " +
            self.cellCheckInterval * config.MIN_INTERVAL
        end
        if self.isStationary then
        ...
        if self.bindToOrder then
            description = description + "\n|cffffcc00Bound to order:|r " + OrderId2String(self.bindToOrder) +
            " |cffaaaaaa(current order = " + OrderId2String(GetUnitCurrentOrder(self.anchor)) + ")|r"
        end
        ...
        if type(self.host) == "table" then
            ...
            for func, __ in pairs(funcs) do
                requiredFieldString = GetMissingRequiredFieldsString(self, func, isMaleInFunc[func], isFemaleInFunc
                [func])
                if requiredFieldString ~= "" then
                    ...
                    description = description + "\n" + requiredFieldString + " |cffaaaaaa(" + Function2String(func) +
                    ")|r"
                end
            end
        end
                    ...
                    if not thisPair.paused then
                        ...
                        elseif thisPair[0x5] == DO_NOT_EVALUATE then
                            ...
                            if functionDelay[0x8] then
                                ...
                                if functionDelayIsDistributed[interactionFunc] then
                                    functionDelayCurrent[interactionFunc] = functionDelay[interactionFunc] +
                                    config.MIN_INTERVAL
                                    if functionDelayCurrent[interactionFunc] > functionDelay[interactionFunc] then
                                        functionDelayCurrent[interactionFunc] = functionDelayCurrent[interactionFunc] -
                                        functionDelay[interactionFunc]
                                    end
                                    nextStep = cycle.counter +
                                    (functionDelayCurrent[interactionFunc] * INV_MIN_INTERVAL + 1) // 1
                                else
                                    nextStep = cycle.counter + (functionDelay[interactionFunc] * INV_MIN_INTERVAL + 1) //
                                    1
                                end
                            else
                                nextStep = cycle.counter + 1
    ...
    local function PrintCrashMessage(crashingPair, A, B)
        ...
        if B == SELF_INTERACTION_ACTOR then
            warning =
                ...
                ..
                "|cffffcc00" ..
                Identifier2String(A.identifier) ..
                "|r. Unique number: " ..
                A.unique ..
                ". The crash occured during self-interaction with function |cffaaaaff" ..
                Function2String(crashingPair[0x8]) .. "|r. The interaction has been disabled."
        else
            warning =
                ...
                Identifier2String(A.identifier) ..
                "|r and |cffaaaaff" ..
                Identifier2String(B.identifier) ..
                "|r. Unique numbers: " .. A.unique .. ", " .. B.unique .. ". The pair has been removed from the cycle."
        end
        ...
        if type(A.host) == "table" then
            ...
            if requiredFieldString ~= "" then
                warning = warning ..
                "\n\nThere are one or more |cffff0000required fields missing|r for the interaction function in the host table of actor " ..
    ...
    local function OnCrash()
        ...
        if A.causedCrash then
            Warning("\nActor with identifier " ..
            Identifier2String(A.identifier) ..
            ", unique number: " .. A.unique .. " is repeatedly causing crashes. Isolating...")
            Suspend(A)
        elseif B.causedCrash and B ~= SELF_INTERACTION_ACTOR then
            Warning("\nActor with identifier " ..
            Identifier2String(B.identifier) ..
            ", unique number: " .. B.unique .. " is repeatedly causing crashes. Isolating...")
            Suspend(B)
        end
    ...
    local function CellCheck()
        ...
        if debug.visualizeAllActors then
            for i = 1, numCellChecks[currentCounter] do
                actor = actorsThisStep[i]
                BlzSetSpecialEffectPosition(actor.visualizer, actor.x[actor.anchor], actor.y[actor.anchor],
                    actor.z[actor] + 75)
            end
        end
    ...
    local function Main()
        ...
        if debug.benchmark then
            ...
            Warning("eval time: |cffffcc00" ..
            string.format("\x25.2f", 1000 * averageEvalTime) ..
            "ms|r, actors: " ..
            #actorList ..
            ", pairs: " ..
            numPairs[currentCounter] + numEveryStepPairs .. ", cell checks: " .. numCellChecks[currentCounter])
        end
    ...
    UpdateSelectedActor = function()
        if debug.selectedActor then
            if not debug.selectedActor.isGlobal then
                ...
                BlzSetSpecialEffectPosition(debug.selectedActor.visualizer, x, y,
                    debug.selectedActor.z[debug.selectedActor] + 75)
    ...
    local function OnMouseClick()
        ...
        local mouseScreenX, mouseScreenY = World2Screen(eyeX, eyeY, eyeZ, angleOfAttack, mouseX, mouseY,
            GetTerrainZ(mouseX, mouseY))
    ...
    local function OnCtrlR()
        Warning("Going to step " ..
        cycle.unboundCounter + 1 ..
        " |cffaaaaaa(" .. string.format("\x25.2f", config.MIN_INTERVAL * (cycle.unboundCounter + 1)) .. "s)|r.")
        Main()
    ...
    EnableDebugMode = function()
        ...
        if not nameFound then
            if GetLocalPlayer() == GetTriggerPlayer() then
                print(
                "|cffff0000Warning:|r You need to set yourself as a map creator in the ALICE config to use debug mode.")
            end
            return
        end
        ...
        if not debug.enabled then
            ...
            BlzTriggerRegisterPlayerKeyEvent(debug.lockSelectionTrigger, GetTriggerPlayer() or Player(0), OSKEY_W, 2,
                true)
            ...
            BlzTriggerRegisterPlayerKeyEvent(debug.printFunctionsTrigger, GetTriggerPlayer() or Player(0), OSKEY_G, 2,
                true)
    ...
    function ALICE_PairGetCoordinates3D()
        ...
        return actorA.x[anchorA], actorA.y[anchorA], actorA.z[actorA], actorB.x[anchorB], actorB.y[anchorB],
            actorB.z[actorB]
    end
    ...
    function ALICE_EnumObjectsInLineSegment(x1, y1, x2, y2, halfWidth, identifier, condition, ...)
        if x2 == x1 then
            return ALICE_EnumObjectsInRect(x1 - halfWidth, min(y1, y2), x1 + halfWidth, max(y1, y2), identifier,
                condition, ...)
        end
    ...
    function ALICE_Statistics()
        ...
        for __, functionType in ipairs(sortedKeys) do
            if (100 * functionCount[functionType] / countActivePairs) > 0.1 then
                statistic = statistic ..
                "\n" ..
                string.format("\x25.2f", 100 * functionCount[functionType] / countActivePairs) ..
                "\x25 |cffffcc00" ..
                Function2String(functionType) .. "|r |cffaaaaaa(" .. functionCount[functionType] .. ")|r"
            end
        end
    ...
    function ALICE_OnWidgetEvent(hookTable)
        ...
        if hookTable.onDeath and not config.UNITS_LEAVE_BEHIND_CORPSES then
            Warning(
            "|cffff0000Warning:|r Attempted to create onDeath unit event hook, but ALICE_UNITS_LEAVE_BEHIND_CORPSES is not enabled. Use onRemove instead.")
        end
    end
    ...
    function ALICE_OnCreationAddInteraction(matchingIdentifier, keyword, interactionFunc)
        ...
        if onCreation.interactions[matchingIdentifier][keyword] then
            Warning("|cffff0000Warning:|r Multiple interactionsFuncs added on creation to " ..
            matchingIdentifier .. " and " .. keyword .. ". Previous entry was overwritten.")
        end
