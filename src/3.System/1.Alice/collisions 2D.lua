...
do
    ...
    function CAT_GizmoBounce2D(A, B)
        ...
        if callback then
            callback(A, B, collisionX, collisionY, perpendicularSpeed, parallelSpeed, totalSpeed, centerOfMassVx,
                centerOfMassVy)
        end
    end
    ...
    function CAT_GizmoImpact2D(A, B)
        ...
        if HandleType[A.visual] == "effect" then
            ...
            BlzSetSpecialEffectPosition(A.visual, xBacktracked, yBacktracked,
                A.z or GetTerrainZ(xBacktracked, yBacktracked))
        end
        ALICE_Kill(A)
    end
    ...
    function CAT_GizmoDevour2D(A, B)
        ...
        if HandleType[B.visual] == "effect" then
            ...
            BlzSetSpecialEffectPosition(A.visual, xBacktracked, yBacktracked,
                B.z or GetTerrainZ(xBacktracked, yBacktracked))
        end
        ALICE_Kill(B)
    end
    ...
    local function WidgetBounce2D(gizmo, widget)
        ...
        if callback then
            callback(gizmo, widget, collisionX, collisionY, perpendicularSpeed, parallelSpeed, totalSpeed, centerOfMassVx,
                centerOfMassVy)
        end
    end
    ...
    local function WidgetImpact2D(gizmo, widget)
        ...
        if HandleType[gizmo.visual] == "effect" then
            ...
            BlzSetSpecialEffectPosition(gizmo.visual, xBacktracked, yBacktracked,
                gizmo.z or GetTerrainZ(xBacktracked, yBacktracked))
        end
        ALICE_Kill(gizmo)
    end
    ...
    local function InitItemCollisionCheck2D(gizmo, item)
        ...
        data.collisionRange = (CAT_Data.WIDGET_TYPE_COLLISION_RADIUS[id] or CAT_Data.DEFAULT_ITEM_COLLISION_RADIUS) +
        gizmo.collisionRadius
        gizmo.maxSpeed = gizmo.maxSpeed or CAT_Data.DEFAULT_GIZMO_MAX_SPEED
    end
