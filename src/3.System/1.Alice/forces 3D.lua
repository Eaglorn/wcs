...
do
    ...
    local function GetObjectCrossSection(object)
        if type(object) == "table" then
            ...
            elseif object.anchor then
                ...
                elseif HandleType[object.anchor] == "unit" then
                    ...
                    return CYLINDER_CROSS_SECTION * collisionSize *
                    (CAT_Data.WIDGET_TYPE_HEIGHT[GetUnitTypeId(object.anchor)] or CAT_Data.DEFAULT_UNIT_HEIGHT_FACTOR * collisionSize)
                else
                    return 0
                end
            else
        ...
        elseif HandleType[object] == "unit" then
            ...
            return CYLINDER_CROSS_SECTION * collisionSize *
            (CAT_Data.WIDGET_TYPE_HEIGHT[GetUnitTypeId(object)] or CAT_Data.DEFAULT_UNIT_HEIGHT_FACTOR * collisionSize)
        else
            return 0
        end
    end
    ...
    local function AccelerateGizmo3D(gizmo)
        ...
        if gizmo.anchor then
            if type(gizmo.anchor) == "table" then
                ...
                CAT_Knockback(gizmo.anchor, gizmo.Fx * gizmo.multiplier, gizmo.Fy * gizmo.multiplier,
                    gizmo.Fz * gizmo.multiplier)
            end
        else
    ...
    local function AccelerateUnit3D(unit)
        ...
        CAT_Knockback(unit, forceTable.Fx * data.multiplier, forceTable.Fy * data.multiplier,
            forceTable.Fz * data.multiplier)
    ...
    Force3D = function(source, target)
        ...
        local interval = (source.intervalFunc(source, target, data, dist, dx, dy, dz, dvx, dvy, dvz) / INTERVAL + 1) *
        INTERVAL
        ...
        local forceXNS, forceYNS, forceZNS = source.forceFunc(source, target, data, distNS, dxNS, dyNS, dzNS, dvx, dvy,
            dvz)
    ...
    local function InitGravity(source, target)
        ...
        data.strength = massSource * massTarget * (GRAVITY_STRENGTH * 1.0001) *
        data.typeMultiplier                                                                 --1.0001 to convert to float to avoid integer overflow
    ...
    local function PressureForce3D(source, target, data, dist, dx, dy, dz, dvx, dvy, dvz)
        ...
        local factor = -source.pressureStrength * data.strengthFactor * (source.pressureMaxRange - dist) /
        (source.pressureMaxRange * dist)
        return dx * factor, dy * factor, dz * factor
    end
    ...
    local function PressureInterval3D(source, target, data, dist, dx, dy, dz, dvx, dvy, dvz)
        ...
        --Arbitrary numbers
        local interval = (0.15 + 0.85 * (dist / source.pressureMaxRange)) * data.intervalFactor / source
        .pressureStrength
    ...
    local function WindForce3D(source, target, data, dist, dx, dy, dz, dvx, dvy, dvz)
        ...
        local factor = dvWindTotal * source.windFieldDensity * data.strengthFactor * (source.windFieldMaxRange - dist) /
        (source.windFieldMaxRange * dist)
    ...
    local function WindFieldInterval3D(source, target, data, dist, dx, dy, dz, dvx, dvy, dvz)
        ...
        local interval = (0.15 + 0.85 * (dist / source.windFieldMaxRange)) * data.intervalFactor /
        source.windFieldDensity
        return interval / (1 + vdivd * interval)
    end
