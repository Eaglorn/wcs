    ...
    OnInit.root("CAT_Data", function()
        ...
        if CAT_Data.CENTER_POINT_COORDINATES then
            ALICE_OnCreationAddFlag("unit", "zOffset", function(host)
                return (CAT_Data.WIDGET_TYPE_HEIGHT[GetUnitTypeId(host)] or CAT_Data.DEFAULT_UNIT_HEIGHT_FACTOR * BlzGetUnitCollisionSize(host)) /
                2
            end)
            ALICE_OnCreationAddFlag("destructable", "zOffset", function(host)
                return (CAT_Data.WIDGET_TYPE_HEIGHT[GetDestructableTypeId(host)] or CAT_Data.DEFAULT_DESTRUCTABLE_HEIGHT) /
                2
            end)
