...
do
            ...
            if self.wallHeight then
                ...
                for i = 1, numEffects + 1 do
                    ...
                    for j = 1, numEffectsVertical + 1 do
                        self.visualizer[i][j] = AddSpecialEffect(
                        "Abilities\\Weapons\\WitchDoctorMissile\\WitchDoctorMissile.mdl", x1 + (i - 1) / numEffects *
                        self.dx, y1 + (i - 1) / numEffects * self.dy)
                        BlzSetSpecialEffectZ(self.visualizer[i][j], self.z + (j - 1) * self.wallHeight /
                        numEffectsVertical)
                    end
                end
            else
                local numEffects = self.length // 64
                for i = 1, numEffects + 1 do
                    self.visualizer[i] = AddSpecialEffect(
                    "Abilities\\Weapons\\WitchDoctorMissile\\WitchDoctorMissile.mdl", x1 + (i - 1) / numEffects * self
                    .dx, y1 + (i - 1) / numEffects * self.dy)
                end
            end
        end
    ...
    function CAT_WallBounce(gizmo, wall)
        ...
        local collisionX, collisionY, collisionZ, perpendicularSpeed, parallelSpeed, totalSpeed = GetImpactParameters(
        gizmo, wall, perpendicularDist)
    ...
    function CAT_WallImpact(gizmo, wall)
        ...
        if HandleType[gizmo.visual] == "effect" then
            BlzSetSpecialEffectPosition(gizmo.visual, collisionX, collisionY,
                gizmo.z or GetTerrainZ(collisionX, collisionY))
        end
    ...
    function CAT_WallCollisionCheck(gizmo, wall)
        ...
        local correctedDist = (perpendicularDist < 0 and -perpendicularDist or perpendicularDist) - gizmo
        .collisionRadius
