...
do
    ...
    local function DamageWidget(gizmo, widget, perpendicularSpeed, parallelSpeed, totalSpeed)
        if HandleType[widget] == "unit" then
            ...
            if type(damage) == "function" then
                UnitDamageTarget(gizmo.source, widget,
                    damage(gizmo, widget, perpendicularSpeed, parallelSpeed, totalSpeed), false, false,
                    gizmo.onUnitAttackType or ATTACK_TYPE_NORMAL, gizmo.onUnitDamageType or DAMAGE_TYPE_MAGIC,
                    WEAPON_TYPE_WHOKNOWS)
            else
                UnitDamageTarget(gizmo.source, widget, damage, false, false, gizmo.onUnitAttackType or ATTACK_TYPE_NORMAL,
                    gizmo.onUnitDamageType or DAMAGE_TYPE_MAGIC, WEAPON_TYPE_WHOKNOWS)
            end
        elseif HandleType[widget] == "destructable" then
            local damage = ALICE_FindField(gizmo.onDestructableDamage, widget)
            if type(damage) == "function" then
                SetDestructableLife(widget,
                    GetDestructableLife(widget) - damage(gizmo, widget, perpendicularSpeed, parallelSpeed, totalSpeed))
            else
            ...
            if type(damage) == "function" then
                SetWidgetLife(widget,
                    GetWidgetLife(widget) - damage(gizmo, widget, perpendicularSpeed, parallelSpeed, totalSpeed))
            else
    ...
    local function GizmoRecoilAndDisplace3D(object, x, y, z, recoilX, recoilY, recoilZ)
        if object.anchor then
            if type(object.anchor) == "table" then
                ...
                object.anchor.vx, object.anchor.vy, object.anchor.vz = object.anchor.vx + recoilX,
                    object.anchor.vy + recoilY, object.anchor.vz + recoilZ
            else
    ...
    function CAT_GizmoBounce3D(A, B)
        ...
        if callback then
            callback(A, B, collisionX, collisionY, collisionZ, perpendicularSpeed, parallelSpeed, totalSpeed,
                centerOfMassVx, centerOfMassVy, centerOfMassVz)
        end
    end
    ...
    function CAT_GizmoImpact3D(A, B)
        ...
        if HandleType[A.visual] == "effect" then
            ...
            local xBacktracked, yBacktracked, zBacktracked = A.x - r * A.vx * INTERVAL, A.y - r * A.vy * INTERVAL,
                A.z - r * A.vz * INTERVAL
    ...
    function CAT_GizmoDevour3D(A, B)
        ...
        if HandleType[B.visual] == "effect" then
            local r = GetBacktrackRatio(dist, collisionDist, dx, dy, dz, dvx, dvy, dvz)
            local xBacktracked, yBacktracked, zBacktracked = B.x - r * B.vx * INTERVAL, B.y - r * B.vy * INTERVAL,
                B.z - r * B.vz * INTERVAL
            BlzSetSpecialEffectPosition(A.visual, xBacktracked, yBacktracked, zBacktracked)
        end
        ALICE_Kill(B)
    ...
    function CAT_GizmoAnnihilate3D(A, B)
        ...
        if HandleType[A.visual] == "effect" then
            xBacktracked, yBacktracked, zBacktracked = A.x - r * A.vx * INTERVAL, A.y - r * A.vy * INTERVAL,
                A.z - r * A.vz * INTERVAL
            BlzSetSpecialEffectPosition(A.visual, xBacktracked, yBacktracked, zBacktracked)
        end
        ALICE_Kill(A)
    ...
    function CAT_GizmoPassThrough3D(A, B)
        ...
        if callback then
            ...
            local collisionX, collisionY, collisionZ, collisionDist = GetGizmoCollisionPoint3D(A, B, xa, ya, za, xb, yb,
                zb)
            callback(A, B, collisionX, collisionY, collisionZ, perpendicularSpeed, parallelSpeed, totalSpeed,
                centerOfMassVx, centerOfMassVy, centerOfMassVz)
        end
    ...
    function CAT_GizmoCylindricalCollisionCheck3D(gizmo, cylinder)
        ...
        return (horiDist - collisionRange) /
        ((gizmo.maxSpeed or CAT_Data.DEFAULT_GIZMO_MAX_SPEED) + (cylinder.maxSpeed or CAT_Data.DEFAULT_GIZMO_MAX_SPEED))
    end
    ...
    function CAT_GizmoCollisionCheck3D(A, B)
        ...
        return (dist - collisionRange) /
        ((A.maxSpeed or CAT_Data.DEFAULT_GIZMO_MAX_SPEED) + (B.maxSpeed or CAT_Data.DEFAULT_GIZMO_MAX_SPEED))
    end
    ...
    local function WidgetBounce3D(gizmo, widget)
        ...
        if HandleType[widget] == "unit" then
            isUnit = true
            elasticity = sqrt((gizmo.elasticity or 1) *
            (CAT_Data.WIDGET_TYPE_ELASTICITY[GetUnitTypeId(widget)] or CAT_Data.DEFAULT_UNIT_ELASTICITY))
            callbackName = "onUnitCallback"
            damageName = "onUnitDamage"
        elseif HandleType[widget] == "destructable" then
            elasticity = sqrt((gizmo.elasticity or 1) *
            (CAT_Data.WIDGET_TYPE_ELASTICITY[GetDestructableTypeId(widget)] or CAT_Data.DEFAULT_DESTRUCTABLE_ELASTICITY))
            callbackName = "onDestructableCallback"
            damageName = "onDestructableDamage"
        else
            elasticity = sqrt((gizmo.elasticity or 1) *
            (CAT_Data.WIDGET_TYPE_ELASTICITY[GetItemTypeId(widget)] or CAT_Data.DEFAULT_ITEM_ELASTICITY))
        ...
        if callback then
            callback(gizmo, widget, collisionX, collisionY, collisionZ, perpendicularSpeed, parallelSpeed, totalSpeed,
                centerOfMassVx, centerOfMassVy, centerOfMassVz)
        end
    end
    ...
    local function WidgetImpact3D(gizmo, widget)
        ...
        if HandleType[gizmo.visual] == "effect" then
            ...
            local xBacktracked, yBacktracked, zBacktracked = gizmo.x - r * gizmo.vx * INTERVAL,
                gizmo.y - r * gizmo.vy * INTERVAL, gizmo.z - r * gizmo.vz * INTERVAL
