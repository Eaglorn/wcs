...
do
    ...
    local function InitMoveArced(missile)
        ...
        local dx, dy, dz = missile.targetX - missile.launchX, missile.targetY - missile.launchY,
            missile.targetZ - missile.launchZ
    ...
    function CAT_MoveHoming2D(missile)
        if ALICE_PairCooldown(0.1) == 0 then
            ...
            elseif diff < 0 then
                ...
                local targetTurnLocDist = sqrt((turnLocX - missile.lastKnownTargetX) ^ 2 +
                (turnLocY - missile.lastKnownTargetY) ^ 2)
        ...
        BlzSetSpecialEffectPosition(missile.visual, missile.x, missile.y,
            GetTerrainZ(missile.x, missile.y) + missile.visualZ)
    end
    ...
    local function InitMoveArcedHoming(missile)
        ...
        missile.turnRateNormalized = bj_DEGTORAD * missile.turnRate * 0.1
        missile.turnRadius = missile.speed / (bj_DEGTORAD * missile.turnRate)
        missile.lastKnownTargetX, missile.lastKnownTargetY, missile.lastKnownTargetZ = ALICE_GetCoordinates3D(missile
        .target)
    ...
    function CAT_MoveArcedHoming(missile)
        if ALICE_PairCooldown(0.1) == 0 then
            ...
            if x ~= 0 or y ~= 0 or z ~= 0 then
                if missile.disconnectionDistance then
                    local dx, dy, dz = x - missile.lastKnownTargetX, y - missile.lastKnownTargetY,
                        z - missile.lastKnownTargetZ
                    ...
                    missile.lastKnownTargetX, missile.lastKnownTargetY, missile.lastKnownTargetZ = ALICE_GetCoordinates3D(
                    missile.target)
                end
            end
            ...
            local dz = missile.lastKnownTargetZ - missile.z +
            missile.verticalArc * (horizontalDist + absDiff * missile.turnRadius)
    ...
    function CAT_MoveHoming3D(missile)
        if ALICE_PairCooldown(0.1) == 0 then
            ...
            local vx, vy, vz = missile.vx, missile.vy, missile.vz
            local dx, dy, dz = missile.lastKnownTargetX - missile.x, missile.lastKnownTargetY - missile.y,
                missile.lastKnownTargetZ - missile.z
            local dist = sqrt(dx * dx + dy * dy + dz * dz)
            ...
            if invNorm ~= math.huge then
                ...
                if dist < 2 * missile.turnRadius then
                    ...
                    ntpx, ntpy, ntpz = ntpx * invNorm + missile.x, ntpy * invNorm + missile.y, ntpz * invNorm + missile
                    .z
                    local targetTurnLocDist = sqrt((missile.lastKnownTargetX - ntpx) ^ 2 +
                    (missile.lastKnownTargetY - ntpy) ^ 2 + (missile.lastKnownTargetZ - ntpz) ^ 2)
                    if targetTurnLocDist > missile.turnRadius or (targetTurnLocDist > 0.9 * missile.turnRadius and missile.isTurning) then
                        missile.isTurning = true
                        missile.x = missile.x + missile.vx * INTERVAL
                        missile.y = missile.y + missile.vy * INTERVAL
