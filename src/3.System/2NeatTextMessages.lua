...
do
    ...
    local VERTICAL_ALIGNMENT                      =
        TEXT_JUSTIFY_MIDDLE ---@type textaligntype --TEXT_JUSTIFY_BOTTOM, TEXT_JUSTIFY_MIDDLE, or TEXT_JUSTIFY_TOP
    local HORIZONTAL_ALIGNMENT                    =
        TEXT_JUSTIFY_CENTER ---@type textaligntype --TEXT_JUSTIFY_LEFT, TEXT_JUSTIFY_CENTER, or TEXT_JUSTIFY_RIGHT
    ...
    TEXT_MESSAGE_X_POSITION                       = 0.225 ---@type number --0 = left, 0.8 = right (bottom-left corner)
    ...
    local TOOLTIP_ABILITY                         =
    'Amls' ---@type string --For REPLACE_BLIZZARD_FUNCTION_CALLS only. Any unused ability for which the library can change the tooltip to extract the TRIGSTR from.
    ...
    ChangeTextFormatting = function(w, whichFrame, whichFormat)
        ...
        BlzFrameSetTextAlignment(w.textCarryingFrame[whichFrame], whichFormat.verticalAlignment,
            whichFormat.horizontalAlignment)
    end
    ...
    ChangeText = function(w, whichFrame, whichText)
        ...
        if w.maxTextMessages == 1 then
            BlzFrameSetSize(w.textCarryingFrame[whichFrame], w.width / (w.messageFormat[whichFrame].fontSize / 10.),
                w.maxHeight / (w.messageFormat[whichFrame].fontSize / 10.))
        else
            BlzFrameSetSize(w.textCarryingFrame[whichFrame], w.width / (w.messageFormat[whichFrame].fontSize / 10.), 0)
        end

        if INCLUDE_FDF then
            BlzFrameSetSize(w.textMessageFrame[whichFrame], BlzFrameGetWidth(w.textMessageText[whichFrame]) + 0.008,
                BlzFrameGetHeight(w.textMessageText[whichFrame]) + 0.009)
            BlzFrameSetPoint(w.textMessageBox[whichFrame], FRAMEPOINT_BOTTOMLEFT, w.textMessageFrame[whichFrame],
                FRAMEPOINT_BOTTOMLEFT, 0, -0.0007 * (w.messageFormat[whichFrame].fontSize - 13))
            BlzFrameSetPoint(w.textMessageBox[whichFrame], FRAMEPOINT_TOPRIGHT, w.textMessageFrame[whichFrame],
                FRAMEPOINT_TOPRIGHT, 0, 0)
        end
                ...
                if w.messageTimeRemaining[i] > 0 then
                    ...
                    if w.messageTimeRemaining[i] < w.messageFormat[i].fadeOutTime then
                        if w.messageTimeRemaining[i] < 0 then
                            ...
                            BlzFrameSetAlpha(w.textMessageFrame[i],
                                math.floor(255 * w.messageTimeRemaining[i] / w.messageFormat[i].fadeOutTime))
                        end
                    elseif w.messageTimeElapsed[i] < w.messageFormat[i].fadeInTime then
                        BlzFrameSetAlpha(w.textMessageFrame[i],
                            math.floor(255 * w.messageTimeElapsed[i] / w.messageFormat[i].fadeInTime))
                    end
    ...
    local function RepositionAllMessages(w)
        ...
        for i = 1, w.maxTextMessages do
            ...
            elseif w.isTopToBottom then
                BlzFrameSetAbsPoint(w.textMessageFrame[i], FRAMEPOINT_BOTTOMLEFT, w.xPosition,
                    w.yPosition - w.textHeight[i] - yOffset[i])
            else
    ...
    local function AddTextMessage(whichText, forcedDuration, whichFormat, w)
        ...
        for i = w.maxTextMessages - 1, 1, -1 do
            ...
            if w.messageTimeRemaining[i + 1] < w.messageFormat[i + 1].fadeOutTime then
                BlzFrameSetAlpha(w.textMessageFrame[i + 1],
                    math.floor(255 * w.messageTimeRemaining[i + 1] / w.messageFormat[i + 1].fadeOutTime))
            else
            ...
            if w.textMessageIconOrientation[i] ~= nil then
                if w.textMessageIconOrientation[i + 1] == "topleft" then
                    BlzFrameSetPoint(w.textMessageIcon[i + 1], FRAMEPOINT_TOPRIGHT, w.textMessageFrame[i + 1],
                        FRAMEPOINT_TOPLEFT, 0, 0)
                elseif w.textMessageIconOrientation[i + 1] == "topright" then
                    BlzFrameSetPoint(w.textMessageIcon[i + 1], FRAMEPOINT_TOPLEFT, w.textMessageFrame[i + 1],
                        FRAMEPOINT_TOPRIGHT, 0, 0)
                elseif w.textMessageIconOrientation[i + 1] == "bottomleft" then
                    BlzFrameSetPoint(w.textMessageIcon[i + 1], FRAMEPOINT_BOTTOMRIGHT, w.textMessageFrame[i + 1],
                        FRAMEPOINT_BOTTOMLEFT, 0, 0)
                elseif w.textMessageIconOrientation[i + 1] == "bottomright" then
                    BlzFrameSetPoint(w.textMessageIcon[i + 1], FRAMEPOINT_BOTTOMLEFT, w.textMessageFrame[i + 1],
        ...
        w.textHeight[1] = BlzFrameGetHeight(w.textMessageFrame[1]) +
            math.max(whichFormat.spacing, w.messageFormat[1].spacing)
        if INCLUDE_FDF then
        ...
        if forcedDuration ~= 0 then
            ...
            w.messageTimeRemaining[1] = whichFormat.minDuration +
                whichFormat.durationIncrease * GetAdjustedStringLength(whichText) + whichFormat.fadeOutTime
        end
    ...
    local function Init()
        ...
        DEFAULT_NEAT_WINDOW = NeatWindow.create(TEXT_MESSAGE_X_POSITION, TEXT_MESSAGE_Y_POSITION,
            TEXT_MESSAGE_BLOCK_WIDTH, TEXT_MESSAGE_BLOCK_MAX_HEIGHT, MAX_TEXT_MESSAGES, MESSAGE_ORDER_TOP_TO_BOTTOM)
    ...
    function EditNeatMessage(messagePointer, newText)
        ...
        whichWindow.textHeight[whichFrame] = BlzFrameGetHeight(whichWindow.textMessageFrame[whichFrame]) +
            math.max(whichWindow.messageFormat[whichFrame].spacing, whichWindow.messageFormat[whichFrame + 1].spacing)
    ...
    function AddNeatMessageTimeRemaining(messagePointer, additionalTime)
        ...
        whichWindow.messageTimeRemaining[whichFrame] = math.max(
            whichWindow.messageTimeRemaining[whichFrame] + additionalTime,
            whichWindow.messageFormat[whichFrame].fadeOutTime)
    end
    ...
    function SetNeatMessageTimeRemaining(messagePointer, newTime)
        ...
        whichWindow.messageTimeRemaining[whichFrame] = math.max(newTime,
            whichWindow.messageFormat[whichFrame].fadeOutTime)
    end
    ...
    function AutoSetNeatMessageTimeRemaining(messagePointer, accountForTimeElapsed)
        ...
        whichWindow.messageTimeRemaining[whichFrame] = whichFormat.minDuration +
            whichFormat.durationIncrease *
            GetAdjustedStringLength(BlzFrameGetText(whichWindow.textCarryingFrame[whichFrame])) + whichFormat
            .fadeOutTime
        if accountForTimeElapsed then
            whichWindow.messageTimeRemaining[whichFrame] = math.max(
                whichWindow.messageTimeRemaining[whichFrame] - whichWindow.messageTimeElapsed[whichFrame],
                whichFormat.fadeOutTime)
        end
    end
    ...
    function NeatMessageAddIcon(messagePointer, width, height, orientation, texture)
        ...
        if orientation == "topleft" then
            BlzFrameSetPoint(whichWindow.textMessageIcon[whichFrame], FRAMEPOINT_TOPRIGHT,
                whichWindow.textMessageFrame[whichFrame], FRAMEPOINT_TOPLEFT, 0, 0)
        elseif orientation == "topright" then
            BlzFrameSetPoint(whichWindow.textMessageIcon[whichFrame], FRAMEPOINT_TOPLEFT,
                whichWindow.textMessageFrame[whichFrame], FRAMEPOINT_TOPRIGHT, 0, 0)
        elseif orientation == "bottomleft" then
            BlzFrameSetPoint(whichWindow.textMessageIcon[whichFrame], FRAMEPOINT_BOTTOMRIGHT,
                whichWindow.textMessageFrame[whichFrame], FRAMEPOINT_BOTTOMLEFT, 0, 0)
        elseif orientation == "bottomright" then
            BlzFrameSetPoint(whichWindow.textMessageIcon[whichFrame], FRAMEPOINT_BOTTOMLEFT,
                whichWindow.textMessageFrame[whichFrame], FRAMEPOINT_BOTTOMRIGHT, 0, 0)
        else
        ...
        if BlzFrameGetHeight(whichWindow.textMessageFrame[whichFrame]) < height then
            BlzFrameSetSize(whichWindow.textCarryingFrame[whichFrame],
                whichWindow.width / (whichWindow.messageFormat[whichFrame].fontSize / 10.), height - 0.018)
            if INCLUDE_FDF then
                BlzFrameSetSize(whichWindow.textMessageFrame[whichFrame],
                    BlzFrameGetWidth(whichWindow.textMessageText[whichFrame]) + 0.008,
                    BlzFrameGetHeight(whichWindow.textMessageText[whichFrame]) + 0.009)
                BlzFrameSetPoint(whichWindow.textMessageBox[whichFrame], FRAMEPOINT_BOTTOMLEFT,
                    whichWindow.textMessageFrame[whichFrame], FRAMEPOINT_BOTTOMLEFT, 0,
                    -0.0007 * (whichWindow.messageFormat[whichFrame].fontSize - 13))
                BlzFrameSetPoint(whichWindow.textMessageBox[whichFrame], FRAMEPOINT_TOPRIGHT,
                    whichWindow.textMessageFrame[whichFrame], FRAMEPOINT_TOPRIGHT, 0, 0)
            end
            whichWindow.textHeight[whichFrame] = BlzFrameGetHeight(whichWindow.textMessageFrame[whichFrame]) +
