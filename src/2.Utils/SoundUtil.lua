do
    ...
    function DestroyLoopingSound(soundpack, fadetime)
        if soundpack.fading ~= nil or soundpack == nil then
            return
        else
            soundpack.fading = true
        end
    ...
    function AddLoopingSoundOnUnit(soundpack, target, fadein_offset, fadeout_offset, delay, volume, cutoff)
        ...
        local function loop_sound()
            ...
            if #soundpack == 1 then
                sound_file = soundpack[1]
            else
                sound_file = soundpack[GetRandomInt(1, #soundpack)]
            end
            ...
            if fadein_offset > 0 then
                SetSoundVolume(soundpack.current_sound, 0)
            else
                SetSoundVolume(soundpack.current_sound, soundpack.current_volume)
            end
