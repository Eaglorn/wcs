...
do
    ...
    function TimerQueueElement.create(timeout, callback, ...)
        ...
        if stackSize == 0 then
            new = setmetatable(
                { timeout = timeout, callback = callback, id = TimerQueueElement.nextId, n = select('#', ...), ... },
                TimerQueueElement)
        else
    ...
    local function recycleTimerQueueElement(timerQueueElement)
        ...
        if stackSize < MAX_STACK_SIZE then
            ...
            timerQueueElement.next, timerQueueElement.callback, timerQueueElement.n, timerQueueElement.timeout, timerQueueElement.enabled, timerQueueElement.id =
    ...
    function TimerQueue:callDelayed(timeout, callback, ...)
        ...
        local current = queue
        local current_timeout = current.next.timeout -
            max(timerGetElapsed(self.timer), 0.)                      -- don't use TimerGetRemaining to prevent bugs for expired and previously paused timers.
    ...
    function TimerQueue:resume()
        if self.paused then
            self.paused = false
            self.queue.next.timeout = self.queue.next.timeout -
                timerGetElapsed(self.timer) --need to restart from 0, because TimerGetElapsed(resumedTimer) is doing so as well after a timer is resumed.
