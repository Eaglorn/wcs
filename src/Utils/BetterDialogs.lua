do
    ...
    local function SetupDialog(whichPlayer, dialogTitle, buttonTitles, buttonTooltips, buttonCallbacks)
        ...
        local dialogHeightEstimate = -BUTTON_Y_OFFSET + DIALOG_BOTTOM_GAP + numberOfButtons * BUTTON_HEIGHT +
            (numberOfButtons - 1) * BUTTON_SPACING
        ...
        local dialogParent = BlzCreateFrame("BetterDialog", BlzGetOriginFrame(ORIGIN_FRAME_WORLD_FRAME, 0), 0, 0) ---@type framehandle
        BlzFrameSetAbsPoint(dialogParent, FRAMEPOINT_TOPLEFT, DIALOG_X - DIALOG_WIDTH / 2, DIALOG_Y + dialogHeight / 2)
        BlzFrameSetAbsPoint(dialogParent, FRAMEPOINT_BOTTOMRIGHT, DIALOG_X + DIALOG_WIDTH / 2, DIALOG_Y - dialogHeight /
            2)
        ...
        BlzFrameSetPoint(dialogTitleFrame, FRAMEPOINT_BOTTOMRIGHT, dialogParent, FRAMEPOINT_TOPRIGHT, 0,
            DIALOG_TITLE_Y_OFFSET - heightDifference / 2)
        ...
        for i = 1, numberOfButtons do
            ...
            if type(buttonTitles[i]) == "table" then
                buttonFrames[i] = BlzCreateFrameByType("GLUETEXTBUTTON", buttonTitles[i][1], dialogParent,
                    "ScriptDialogButton", 0)
                isCycleButton = true
                BlzFrameSetText(buttonFrames[i], buttonTitles[i][1])
                cycleButtonState = 1
            elseif type(buttonTitles[i]) == "string" then
                buttonFrames[i] = BlzCreateFrameByType("GLUETEXTBUTTON", buttonTitles[i], dialogParent,
                    "ScriptDialogButton", 0)
                isCycleButton = false
                BlzFrameSetText(buttonFrames[i], buttonTitles[i])
            end
            BlzFrameSetPoint(buttonFrames[i], FRAMEPOINT_TOPLEFT, dialogParent, FRAMEPOINT_TOPLEFT, BUTTON_X_OFFSET,
                BUTTON_Y_OFFSET - (i - 1) * (BUTTON_HEIGHT + BUTTON_SPACING) - heightDifference / 2)
            BlzFrameSetPoint(buttonFrames[i], FRAMEPOINT_BOTTOMRIGHT, dialogParent, FRAMEPOINT_TOPLEFT,
                BUTTON_X_OFFSET + BUTTON_WIDTH,
                BUTTON_Y_OFFSET - BUTTON_HEIGHT - (i - 1) * (BUTTON_HEIGHT + BUTTON_SPACING) - heightDifference / 2)
            ...
            if buttonTooltips[i] ~= "" then
                buttonTooltipFrames[i] = BlzCreateFrame("BetterDialogTooltip" .. TOOLTIP_STYLE,
                    BlzGetOriginFrame(ORIGIN_FRAME_WORLD_FRAME, 0), 0, 0)
                BlzFrameSetAbsPoint(buttonTooltipFrames[i], FRAMEPOINT_TOPLEFT, DIALOG_X + DIALOG_WIDTH / 2,
                    DIALOG_Y + dialogHeight / 2 + TOOLTIP_Y_OFFSET)
                ...
                BlzFrameSetSize(buttonTooltipFrames[i], TOOLTIP_WIDTH,
                    BlzFrameGetHeight(tooltipText) + TOOLTIP_FRAME_HEIGHT_BUFFER)
            end
