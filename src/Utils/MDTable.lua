...
do
    ...
    MDTable.__index = function(t, key)
        ...
        if tMetaInfo.depth == tMetaInfo.maxDepth then
            ...
            tMetaInfo[tMetaInfo.depth] =
                key --add current accessed key to the table to add it to the unpacking. This will be overwritten on every call, but doesn't matter.
            local defaultVal = (type(default) == 'function' and default(unpack(tMetaInfo, 1, tMetaInfo.depth))) or
                default --return default-value for full level read access
            ...
            local newMetaInfo = {
                ...
                ,
                persistDefault = tMetaInfo
                    .persistDefault --pass information on whether to persist default value to subtable
            }
            ...
            return
                newDim                         --return new subtable, so the read access can continue as normal (and probably involves further nested calls of __index)
        end
    end
    ...
    loopRecursion = function(t, syncedLoop_yn, returnVals, loopLevel)
        local tMetaInfo = metaInfo[t]
        local iter = syncedLoop_yn and tMetaInfo.oldPairs() or
            next             --iterator to be used depends on whether we deal with a normal table or a SyncedTable.
    ...
    local syncedIndex = function(t, key)
        ...
        elseif tMetaInfo.depth >= tMetaInfo.maxDepth then
            local default = tMetaInfo.default
            tMetaInfo[tMetaInfo.depth] =
                key --add current accessed key to tMetaInfo to add it to the unpacking below. The key at full depth will be overwritten on every call, but doesn't matter.
            local defaultVal = (type(default) == 'function' and default(unpack(tMetaInfo, 1, tMetaInfo.depth))) or
                default
            if tMetaInfo.persistDefault then
    ...
    prepareSyncedTable = function(t)
        ...
        tMetaInfo.oldPairs = mt
            .__pairs                    --original __pairs-function must be saved somewhere to be accessible from within the loop recursion
    ...
    MDTable.create = function(numDimensions, defaultValue, persistDefault_yn, syncedLoop_yn)
        ...
        metaInfo[newTable].persistDefault =
            persistDefault_yn                       --tells, if the default value shall be saved upon read access
        return newTable
    end
end
